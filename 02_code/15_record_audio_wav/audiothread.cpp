...
void AudioThread::run() {
    ...
    while (!isInterruptionRequested()) {
        ...
        if (ret == 0) {// 读取成功
            ...
            // 整数除以整数，误差会比较大，所以需要乘以浮点数，变成浮点数除以整数，得到一个浮点毫秒数
            unsigned long long ms = 1000.0 * header.dataChunkDataSize / header.byteRate;
            // 发送信号
            qDebug() << header.dataChunkDataSize << header.byteRate << ms;
            emit timeChanged(ms);
...
//    qDebug() << "整个文件大小: " << file.size() << "PCM数据大小: " << header.dataChunkDataSize;
...
//    header.dataChunkDataSize = size - sizeof (WAVHeader);
    ...
    // 根据dataChunkDataSize地址值找到dataChunkDataSize的这块内存空间），
    // 这块内存空间有多少字节要写入文件呢？就是sizeof(...)的值，假设是4个字节，
    // 此时，就会将dataChunkDataSize这块内存空间的4个字节写到文件里面去。
